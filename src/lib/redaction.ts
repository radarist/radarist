/**
 * Secret redaction for logs, error envelopes, and support/debug exports (SEC-013).
 *
 * Canonical implementation. `agent/src/redaction.ts` is a hand-synced mirror for
 * the `@impulse/agent` sub-package (which cannot import from `src/lib`); the rule
 * tables in both files are compared byte-for-byte by
 * `src/lib/__tests__/redaction-agent-mirror.test.ts`, so a change here fails the
 * build until the mirror is updated too.
 *
 ...
 *    This is the net that actually proves "no token/key/header value appears in
 *    the log", because it does not depend on recognising the credential format.
 ...
 *    private keys, `Bearer …`, URL userinfo, `?api_key=…`) are masked in free
 *    text, which covers third-party secrets we never named.
 *
 * The module is dependency-free and safe in both client and server bundles.
 */
...
// ---------------------------------------------------------------------------
// SHARED RULE TABLES — keep byte-identical with agent/src/redaction.ts
// ---------------------------------------------------------------------------
// #region shared-rules
